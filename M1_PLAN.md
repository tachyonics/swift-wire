...
**Validation gate:** two-package test setup mirroring task-cluster's structure (consumer + external library); the consumer activates the library by depending on it; bindings from both compose — including a genuine cross-module name clash resolved via `::` and a cross-module key reference; removing the dependency (de-activating) produces a clear missing-binding error at the relevant `@Inject` with a fix-it suggesting the dependency. A binding consumed across the module boundary at less than `public`/`package` produces the context-dependent visibility error. Single-`BindingKey` tracking diagnostics (7a) are covered by their own single-module fixtures. (Eager construction of all activated bindings is accepted in M1; the reachability-pruning optimization is M6b.)
