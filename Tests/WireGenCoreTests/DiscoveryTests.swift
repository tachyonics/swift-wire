...
struct DiscoveryTests {
    ...
    // MARK: - `Lazy<T>` wrapper recognition

    @Test func injectPropertyWithLazyWrapperUnwrapsToInnerType() {
        // `@Inject var x: Lazy<Database>` — discovery unwraps to
        // dep type "Database" with `isLazyWrapped: true`. Graph
        // identity is the inner type so cycle/missing-binding/
        // cross-scope all apply against `Database`, not against
        // `Lazy<Database>`.
        let source = """
            @Singleton
            struct Consumer {
                @Inject var db: Lazy<Database>
            }
            """
        let result = discoverSingletons(in: source, sourcePath: "Consumer.swift")
        #expect(result.count == 1)
        #expect(result[0].dependencies.count == 1)
        let dep = result[0].dependencies[0]
        #expect(dep.type == "Database")
        #expect(dep.isLazyWrapped == true)
    }

    @Test func injectPropertyWithoutLazyHasFalseFlag() {
        // Sanity check the other direction: a plain `@Inject var
        // x: Database` produces a dep with `isLazyWrapped: false`.
        let source = """
            @Singleton
            struct Consumer {
        ...
        let dep = result[0].dependencies[0]
        #expect(dep.type == "Database")
        #expect(dep.isLazyWrapped == false)
    }

    @Test func injectInitParameterWithLazyWrapperUnwrapsAndFlags() {
        let source = """
            @Singleton
            struct Consumer {
                @Inject init(db: Lazy<Database>) {}
            }
            """
        let result = discoverSingletons(in: source, sourcePath: "Consumer.swift")
        #expect(result.count == 1)
    ...
    @Test func providesFunctionParameterWithLazyWrapperUnwrapsAndFlags() {
        let source = """
            @Provides
            func makeService(db: Lazy<Database>) -> Service { Service() }
            """
        let result = discoverProviders(in: source, sourcePath: "Source.swift")
        #expect(result.count == 1)
        #expect(result[0].dependencies.count == 1)
    ...
    @Test func lazyWithGenericInnerTypeUnwraps() {
        // `Lazy<Repository<User>>` — the inner type is itself a
        // generic instantiation. Unwrap to the inner verbatim;
        // generic-specialisation downstream handles
        // `Repository<User>` as it would any other instantiated
        // generic.
        let source = """
            @Singleton
            struct Consumer {
                @Inject var repo: Lazy<Repository<User>>
            }
            """
        let result = discoverSingletons(in: source, sourcePath: "Consumer.swift")
        #expect(result.count == 1)
        let dep = result[0].dependencies[0]
        #expect(dep.type == "Repository<User>")
        #expect(dep.isLazyWrapped == true)
    }

    @Test func lazyWrapperInsideAnotherGenericDoesNotMatch() {
        // `Box<Lazy<Database>>` — the outer type is `Box`, not
        // `Lazy`. Discovery doesn't recurse into generic arguments
        // hunting for Lazy wrappers; the dep keeps the full type
        // verbatim and Wire treats it as a binding of
        // `Box<Lazy<Database>>` shape.
        let source = """
            @Singleton
            struct Consumer {
                @Inject var boxedDb: Box<Lazy<Database>>
            }
            """
        let result = discoverSingletons(in: source, sourcePath: "Consumer.swift")
        #expect(result.count == 1)
        let dep = result[0].dependencies[0]
        #expect(dep.type == "Box<Lazy<Database>>")
        #expect(dep.isLazyWrapped == false)
    }

    @Test func doubleLazyDoesNotRecursivelyUnwrap() {
        // `Lazy<Lazy<Database>>` — the outer Lazy is recognised
        // and unwrapped, but the inner stays as `Lazy<Database>`.
        // The graph then looks for a binding of type
        // `Lazy<Database>` — the user is presumably providing one
        // via `@Provides`. Matches the design-note posture:
        // double-deferral is a rare edge case we don't try to
        // collapse implicitly.
        let source = """
            @Singleton
            struct Consumer {
                @Inject var nested: Lazy<Lazy<Database>>
            }
            """
        let result = discoverSingletons(in: source, sourcePath: "Consumer.swift")
        #expect(result.count == 1)
        let dep = result[0].dependencies[0]
        #expect(dep.type == "Lazy<Database>")
        #expect(dep.isLazyWrapped == true)
    }
