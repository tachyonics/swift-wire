...
struct WireGen {
    static func main() throws {
        ...
        // factory types (so its own controllers' wrapping inits resolve) + a Wire-aware marker. No
        // graph, no bootstrap. Graph consumers (the default) build the full graph.
...
/// Library mode — emit a Wire-aware module's owned factory types + a marker, no graph.
/// `WireGen --library <factory-output> <exports-output> --module <self> <sources…>`. Only the
/// module's own sources are read: factory types are template-driven (own-module) and need no
/// dependency graph. The module's `import`s are propagated so foreign dependency types the
/// factories reference resolve.
private func runLibraryMode(arguments: [String]) throws {
    guard arguments.count >= 4 else { WireGen.printUsageAndExit() }
    let factoryOutputPath = arguments[2]
    let exportsOutputPath = arguments[3]
    let groups = WireGen.parseModuleGroups(Array(arguments.dropFirst(4)))
    ...
    // Wire-aware marker. The export interface — what this module contributes to a consumer's graph —
    // is deferred to M6; for now the file's presence is the signal, empty of content.
    try "// Generated by WireGen — Wire-aware module marker. Export interface: M6.\n"
        .write(toFile: exportsOutputPath, atomically: true, encoding: .utf8)
    print("wrote \(exportsOutputPath)")
}
