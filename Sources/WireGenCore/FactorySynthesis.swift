...
package struct SynthesizedFactory: Sendable {
    ...
    /// The module the produced (template) type lives in — the generated file needs
    /// an `import` for it when it's foreign.
...
package func synthesizeFactories(
    ...
    return consumedKeys.sorted().compactMap { key in
        guard let template = templatesByKey[key] else { return nil }
...
package func renderFactoryDeclaration(_ factory: SynthesizedFactory) -> String {
    ...
    var lines: [String] = []
    lines.append("struct \(factory.factoryTypeName) {")
