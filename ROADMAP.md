...
M0, M1, and M2 are complete. The historical implementation plans are archived at
[Documentation/Archive/M1_PLAN.md](Documentation/Archive/M1_PLAN.md) and
[Documentation/Archive/M2_PLAN.md](Documentation/Archive/M2_PLAN.md); the detail
...
- **M2: `WireHummingbird` adapter — complete.** The first framework adapter: native **app-scoped** Hummingbird controllers auto-wired onto a `Router` that stays *outside* the graph, on the principle of **collation, not registration**. Two framework-agnostic capabilities landed in Wire Core — the graph-conformance emission (`extension _WireGraph: <Protocol>`, so Wire wires an adapter-declared protocol knowing nothing about HTTP) and the **contribution-alias** adapter contract (an annotation aliases `@Contributes(to: key)`, retiring the iteration-8 `_wireRegister` side-effect). On top, the external `wire-hummingbird` repo (depending on pushed swift-wire main) ships: context-free route collation via `@HummingbirdRoute` (a `@Contributes` alias that also generates the mount witness); service-lifecycle collation via `@HummingbirdService` → `[any Service]` (the first real `CollectedKey` consumer); and a framework-agnostic `introspect()` wiring model (bindings, kinds, scopes, dependency edges, source locations) with a mountable JSON endpoint. **Middleware is out of scope** — a context-typed value with no clean collation shape, so the app owns it via `router.addMiddleware`; the callable-vs-value boundary (routes/controllers are callables whose context defers to the call site; middleware and typed values aren't) is what decides what collates. The **Tier-2 `@WireHummingbird` composition-root macro is deferred to M5** (below). See the archived [M2 plan](Documentation/Archive/M2_PLAN.md) and [WireHummingbirdDesign.md](Documentation/Notes/WireHummingbirdDesign.md).
- **M3: `WireOpenAPI` adapter (`@OpenAPIController`).** The **cross-runtime** adapter and the headline differentiator. Lands when task-cluster's `TaskController.registerHandlers(on: router)` call moves into the adapter-annotation system. It **re-homes M2's collation model** from `some RouterMethods<Context>` onto `some ServerTransport`: `@OpenAPIController` (a `@Contributes` alias, mirroring `@HummingbirdRoute` — optional path → `registerHandlers`'s `serverURL`) makes an `APIProtocol` conformer a `TransportContributor` whose generated witness calls `registerHandlers`; a facade applies the collated handlers to any `ServerTransport`. Because the target is `ServerTransport` (and the package depends only on `OpenAPIRuntime`, no HTTP framework), the same wired controller mounts on Hummingbird, Vapor, or Lambda unchanged. **Handlers-only** — unlike Hummingbird, OpenAPI is not a runtime, so services/lifecycle stay with WireHummingbird (both adapters coexist on one graph). Establishes the `ServerTransport` collation surface (`TransportContributor` / `TransportKeys.handlers` / `TransportComposable` / `apply`) that M5's `@Controller` folds into — a re-home, not a parallel surface. `@OpenAPIConfiguration` and middleware are deferred (middleware to align with M5's `swift-http-api-proposal` `Middleware`). See [WireOpenAPIDesign.md](Documentation/Notes/WireOpenAPIDesign.md).
...
- **M5: `WireMVC` adapter.** Lands when task-cluster has an actual use case for inline route declarations — likely an internal admin endpoint, or as a deliberate content piece contrasting the OpenAPI adapter. The first type-level-with-member-recognition adapter; if the contract holds up here, it'll hold up for almost anything. **Absorbs work deferred from M2:**
  - **A native-middleware router PoC**, then the **Tier-2 `@WireHummingbird` composition-root macro** (M5.2) — the macro codifies the app-assembly shape (`bootstrap → router → apply → Application → run`), which is exactly what WireMVC's routing/middleware work may perturb, so it's built once the shape is settled rather than rewritten.
  - **The standard-`Middleware` cross-runtime path** — the ecosystem-standard [`Middleware`](https://github.com/apple/swift-http-api-proposal/blob/main/Sources/Middleware/Middleware.swift) (parameterised by input/output only, no framework context) *is* collatable, with WireMVC owning an aggregation key and an adapter declaring the concrete type; applying that stack likely needs a custom router/transport. This is where the `BuilderKey`→opaque-member graph-conformance emission (shipped but unused in M2) first gets exercised.
