...
M0 through M4 are complete. The historical implementation plans are archived at
[Documentation/Archive/M1_PLAN.md](Documentation/Archive/M1_PLAN.md) and
[Documentation/Archive/M2_PLAN.md](Documentation/Archive/M2_PLAN.md) (M3's design lives
in [WireOpenAPIDesign.md](Documentation/Notes/WireOpenAPIDesign.md)); M5's active plan
is [Documentation/M5_PLAN.md](Documentation/M5_PLAN.md); the detail
...
- **M4: lifecycle orchestration — complete.** The forcing case: task-cluster moved onto a real (Soto) DynamoDB client vs the in-memory one, needing orderly shutdown. The `@Teardown` annotation existed from M1 (recognised and recorded, but inert); M4 emits the **app-scope** teardown walk — `teardown()` on the generated graph, calling each `@Teardown` action in reverse dependency order, run at shutdown via WireHummingbird's `teardownService` (a `ServiceLifecycle` service prepended so it shuts down last, after the server stops). Teardown-action failures are collected and logged so one doesn't stop the rest. **Request-/job-scope teardown** needs request scope and is **M5**. **Init-failure partial teardown** — tearing down already-constructed bindings in reverse before a bootstrap rethrow — is deferred to **M6c**: its implementation is fixed by the construction scheduler that pass settles (a linear prefix today vs. resolved `AtomicState` cells under dynamic scheduling), so it lands there once rather than being rewritten; until then a bootstrap init-failure leaves constructed resources for process exit to reclaim. The forcing case moved task-cluster onto the Soto AWS stack (`AWSClient` `@Provides @Teardown`), validated against a real table via a LocalStack integration test. See [TeardownDesign.md](Documentation/Notes/TeardownDesign.md).
- **M5: `WireMVC` adapter.** Lands when task-cluster has an actual use case for inline route declarations — likely an internal admin endpoint, or as a deliberate content piece contrasting the OpenAPI adapter. The first type-level-with-member-recognition adapter (spike-2 proved the macro mechanics); if the contract holds up here, it'll hold up for almost anything. `@Controller`/`@Get`/`@Path`/`@JSONResponse` **fold into a Wire collation surface** (`WireMVCKeys.routeContributors`, mirroring M3's `ServerTransport` collation *shape* with WireMVC's own key) — so cross-runtime comes for free and WireMVC is essentially a spec-free, annotation-driven analogue of the OpenAPI generator's registration codegen. **Proposal-native pivot:** the witness registers on `RoutableHTTPServerBuilder` (over `swift-http-api-proposal`'s `HTTPServer`), not `some ServerTransport` — deploying against macOS 26 makes `anyAppleOS 26.0` unconditional, so the plan's *tracked successor* became the core; `some ServerTransport` is retained as the opt-in `WireMVCServerTransport` adapter (Hummingbird/Vapor), so the core no longer depends on OpenAPIRuntime. The implementation plan is [Documentation/M5_PLAN.md](Documentation/M5_PLAN.md); the settled M5.0 decisions and annotation surface are the authoritative record in [Documentation/Notes/WireMVCDesign.md](Documentation/Notes/WireMVCDesign.md), and the M5.2/M5.3 raw-handler + middleware design in [Documentation/Notes/WireMVCMiddleware.md](Documentation/Notes/WireMVCMiddleware.md) (all reconciled to the pivot; proven by spikes 11–14). **Absorbs work deferred from M2:**
  - **Middleware folds into the generated routing — no native-middleware router.** Because WireMVC owns the route-registration codegen, controller- and route-scoped `@Middleware` are nested wrappers around the generated handler closure; composition is closure nesting, no runtime router type exists, and WireMVC composes its own decoded chain rather than a framework's `addMiddleware` (Hummingbird's `RouterMiddleware` is bidirectional/context-typed — incompatible). **Type-transforming middleware falls out as a compile error, not a declared feature:** the codegen threads each middleware's output type into the next stage's input, terminating at the handler's expected input, so an auth middleware that produces an authenticated principal a handler requires either type-checks or fails to compile at the generated seam. The ecosystem-standard proposal `Middleware<Input, NextInput>` is exactly this shape — forward `Input → NextInput` transform with the handler as the terminal stage (`NextInput == Void`), the response carried in `Input`, and the type-mismatch enforced by the standard `@MiddlewareBuilder`/`ChainedMiddleware` (`First.NextInput == Second.Input`) so it fails to compile — which *validates* the property; but it's raw-transport-level and future (`anyAppleOS 26.0`+ server / `26.2` middleware, Swift 6.4 toolchain), so M5 uses a WireMVC *decoded* fold modeled on it over `RoutableHTTPServerBuilder`, adopting the proposal chain as substrate once the deployment floor reaches `26.2`. This is the middleware analogue of the `@Scoped`-controller critique of the request god-object, and it retires the "native-middleware router PoC" the M2 deferral parked. **Global middleware is a separate, deferred concern** (its pre-routing / unmatched-request coverage puts it at the Tier-2 router-assembly layer): default to leaving it a framework concern (the user's `router.addMiddleware`), with a context-free responder/transport decorator as the eventual cross-runtime path when a use case forces it — never a collated `RouterMiddleware` stack.
  - **An escape-hatch raw handler** — a catch-all route whose handler takes the proposal's raw primitives (the `RoutableHTTPServerBuilder` handler signature: `consuming sending Reader`/`ResponseSender`) and is registered verbatim, skipping param-decode/response-encode. Streaming, SSE, and proxying live here rather than in WireMVC's typed core (spike-14 proves SSE streams through it both natively and via the `ServerTransport` adapter); middleware can still wrap them. **WebSocket is the exception** — an upgrade isn't a request→response body, so it's escape-to-framework, not a WireMVC route.
  - **The standard-`Middleware` cross-runtime path** — the ecosystem-standard [`Middleware`](https://github.com/apple/swift-http-api-proposal/blob/main/Sources/Middleware/Middleware.swift) (parameterised by input/output only, no framework context) *is* collatable, with WireMVC owning an aggregation key and an adapter declaring the concrete type. This is where the `BuilderKey`→opaque-member graph-conformance emission (shipped but unused in M2) first gets exercised.
  - **The Tier-2 `@WireHummingbird` composition-root macro** (M5.5) — codifies the app-assembly shape (`bootstrap → router → apply → Application → run`), built once the routing/middleware shape is settled rather than rewritten (the M2.6 deferral).
  - **Request-scoped controllers** — a `@Scoped(seed:)` controller becomes an app-scoped proxy contributor whose *generated* registration embeds per-request scope entry (the M2-deferred mechanism: weak back-ref to the app graph + `bootstrap<Seed>Scope`), reusing the shared "adapter replaces the binding" primitive. Each request-scoped controller is a **per-request reachability root**: its scope-entry constructs only its own transitive request-scoped subgraph, so a request to controller A never builds a request-scoped binding only B reaches (the same reachability concept as M6b, at the request-construction layer rather than build-time singleton pruning — and structural here, not deferrable).
  - **`WireMVCAbstraction.md` rewrite** — the authoritative record is now [WireMVCDesign.md](Documentation/Notes/WireMVCDesign.md); M5.6 folds the abstraction note's remaining Tier-1/Tier-2 progressive-adoption content into it and retires the `_wireRegister` model.
- **M6: performance optimizations.** A cluster of perf passes, each landing when its cost is felt — the multi-module discovery/pruning optimizations (M6a/M6b; multi-module composition itself ships in M1) plus construction scheduling (M6c). All keep the surface contract unchanged and are invisible to users. When M6 starts, the ordering among them is decided then.
...
[OpaqueTypesSupport.md](Documentation/Notes/OpaqueTypesSupport.md) — *Deferred to M2* and *Second
forcing condition*.
